...
const WastagePage = () => {
  ...
  const { products, warehouses, wastage } = useSelector(
  ...
  const onSubmit: SubmitHandler<WastageFormData> = (data) => {
    ...
    if (!selectedProduct || !selectedWarehouseId || wastageQuantity <= 0)
      return;
  ...
  const reportHeadCells: HeadCell<(typeof wastage)[0]>[] = [
    {
      ...
      numeric: false,
      cell: (row) =>
        products.find((p) => p.id === row.productId)?.name || "یافت نشد",
    },
    {
      id: "warehouseId",
      label: "انبار",
      numeric: false,
      cell: (row) =>
        warehouses.find((w) => w.id === row.warehouseId)?.name || "یافت نشد",
    },
    ...
    return (
      <Paper sx={{ p: 2 }}>
        ...
        >
          ...
          <Button variant="contained" onClick={() => setViewMode("form")}>
            ثبت ضایعات جدید
          </Button>
        </Box>
        <EnhancedMuiTable rows={wastage} headCells={reportHeadCells} title="" />
      </Paper>
    );
  }
