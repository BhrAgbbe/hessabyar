...
const CompanyAccountPage = () => {
  ...
  const [transactionModalView, setTransactionModalView] = useState<
    "form" | "report"
  >("form");
  ...
  const {
    ...
    resolver: yupResolver(
      bankAccountSchema as yup.ObjectSchema<BankAccountFormData>
    ),
    defaultValues: {
      bankName: "",
      branchName: "",
  ...
  const confirmAccountDelete = () => {
    accountDeleteConfirm.ids.forEach((id) =>
      dispatch(deleteAccount(String(id)))
    );
  ...
  const onTransactionSubmit: SubmitHandler<TransactionFormData> = (data) => {
    ...
    if (editingTransaction) {
      ...
      const originalBalanceChange =
        originalTransaction.type === "receipt"
          ? originalAmount
          : -originalAmount;
  ...
  const accountHeadCells = useMemo<readonly HeadCell<BankAccount>[]>(
    () => [
      ...
      { id: "branchName", numeric: false, label: "نام شعبه" },
      {
        ...
        numeric: false,
        label: "شماره حساب",
        cell: (row) => toPersianDigits(row.accountNumber),
      },
      {
        id: "balance",
        numeric: true,
        label: "موجودی (تومان)",
        cell: (row) => row.balance.toLocaleString("fa-IR"),
      },
    ],
    []
  );
    ...
    () => [
      {
        ...
        tooltip: "ویرایش",
        onClick: (row) => handleOpenAccountForm(row),
      },
      {
        icon: <DeleteIcon color="error" fontSize="small" />,
        tooltip: "حذف",
        onClick: (row) => handleAccountDeleteRequest([row.id]),
      },
    ],
  ...
  const transactionHeadCells = useMemo<readonly HeadCell<Transaction>[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        numeric: true,
        label: "مبلغ (تومان)",
        cell: (row) => row.amount.toLocaleString("fa-IR"),
      },
    ];
  }, [accounts]);
    ...
    () => [
      {
        icon: <EditIcon fontSize="small" />,
        tooltip: "ویرایش",
        onClick: (row) => setEditingTransaction(row),
      },
      {
        icon: <DeleteIcon color="error" fontSize="small" />,
        tooltip: "حذف",
        onClick: (row) => handleTransactionDeleteRequest([row.id]),
      },
    ],
  ...
  const transactionFormConfig: FormField<TransactionFormData>[] =
    useMemo(() => {
        ...
        <Box>
          <EnhancedMuiTable
            ...
            onDelete={(selectedIds) =>
              handleTransactionDeleteRequest(selectedIds as string[])
            }
          />
          <Button
            variant="contained"
            onClick={() => setTransactionModalView("form")}
            sx={{ mt: 2 }}
          >
            بازگشت به فرم
          </Button>
  ...
  return (
    <Box>
      ...
      >
        ...
        <Button
          variant="text"
          onClick={() => setNewBankDialogOpen(true)}
          sx={{ mt: 2, alignSelf: "flex-start" }}
        >
          افزودن بانک جدید
        </Button>
      ...
      <NewBankDialog
        open={newBankDialogOpen}
        onClose={() => setNewBankDialogOpen(false)}
        onSave={handleAddNewBank}
      />
      <ConfirmationDialog
        open={accountDeleteConfirm.open}
        onClose={() =>
          setAccountDeleteConfirm({ ...accountDeleteConfirm, open: false })
        }
        onConfirm={confirmAccountDelete}
        title="تایید حذف حساب"
        message={`آیا از حذف ${toPersianDigits(
          accountDeleteConfirm.ids.length
        )} حساب بانکی اطمینان دارید؟`}
      />
      ...
      <FormDialog
        ...
        title={
          editingTransaction
            ? "ویرایش تراکنش"
            : transactionModalView === "form"
            ? "ثبت واریز یا برداشت جدید"
            : "گزارش تراکنش‌ها"
        }
      ...
      <ConfirmationDialog
        ...
        message={`آیا از حذف ${toPersianDigits(
          transactionDeleteConfirm.ids.length
