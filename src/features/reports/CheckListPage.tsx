...
const CheckListPage = () => {
  ...
  const sortOptions = [
    ...
    { value: "payee", label: "پرداخت کننده" },
    { value: "amountHigh", label: "بیشترین مبلغ" },
    { value: "amountLow", label: "کمترین مبلغ" },
  ];

  const headCells: readonly HeadCell<Check>[] = [
    { id: "serial", numeric: false, label: "سریال", align: "center" },
    { id: "payee", numeric: false, label: "پرداخت کننده", align: "center" },
    {
      id: "amount",
      numeric: true,
      label: "مبلغ (تومان)",
  ...
  const handleSave = (formData: Omit<Check, "id" | "type">) => {
    if (editingCheck) {
      dispatch(
        editCheck({ ...formData, id: editingCheck.id, type: "received" })
      );
      showToast("چک با موفقیت ویرایش شد.", "success");
    } else {
      dispatch(addCheck({ ...formData, type: "received" }));
...
const CheckForm: React.FC<CheckFormProps> = ({
  ...
  return (
    <FormDialog
      ...
      title={check ? "ویرایش چک" : "افزودن چک جدید"}
    >
      <TextField
        label="سریال"
