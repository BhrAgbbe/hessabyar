...
const CustomerLedgerPage = () => {
  ...
  const headCells: readonly HeadCell<(typeof ledgerEntries)[0]>[] = [
    ...
    },
    {
      ...
      numeric: true,
      label: "بدهکار",
      cell: (row) => toPersianDigits(row.debit || 0),
      align: "center",
      width: "15%",
    },
    {
      ...
      numeric: true,
      label: "بستانکار",
      cell: (row) => toPersianDigits(row.credit || 0),
      align: "center",
      width: "15%",
    },
    {
      ...
      numeric: true,
      label: "مانده",
      cell: (row) => toPersianDigits(row.balance || 0),
      align: "center",
      width: "15%",
    },
  ];
  ...
  const formFields: FormField<TransactionFormData>[] = [
    ...
    },
    {
      ...
      rules: { required: "انتخاب حساب الزامی است" },
    },
    {
      name: "amount",
      label: "مبلغ",
      type: "number",
      rules: { required: "مبلغ الزامی است", valueAsNumber: true },
    },
  ...
  const handleRowClick = (row: (typeof ledgerEntries)[0]) => {
    if (row.type === "sale")
      setSelectedInvoice(
        invoices.sales.find((inv) => inv.id === row.id) || null
      );
    else if (row.type === "return")
      setSelectedInvoice(
        invoices.salesReturns.find((inv) => inv.id === row.id) || null
      );
  };
  ...
  const handleConfirmDelete = () => {
    ...
    else if (item.type === "sale" || item.type === "return")
