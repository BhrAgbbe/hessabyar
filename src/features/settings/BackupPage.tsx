...
const BackupPage = () => {
  ...
  const handleSaveBackup = () => {
    try {
        ...
        link.click();
        setSnackbar({open: true, message: 'فایل پشتیبان با موفقیت دانلود شد.', severity: 'success'});
    } catch  {
        setSnackbar({open: true, message: 'خطا در ایجاد فایل پشتیبان.', severity: 'error'});
    }
  };
        ...
        fileReader.onload = e => {
            if (e.target?.result) {
                try {
                    ...
                    alert('برای بازیابی کامل، باید تمام اکشن‌های setAll در Redux پیاده‌سازی شوند. این بخش به عنوان نمونه است.');
                    setSnackbar({open: true, message: 'اطلاعات با موفقیت بازیابی شد. لطفاً صفحه را رفرش کنید.', severity: 'success'});
                } catch {
                    setSnackbar({open: true, message: 'فایل پشتیبان نامعتبر است.', severity: 'error'});
                }
            }
        };
    }
  ...
  return (
    <Box>
      <Typography variant="h4" gutterBottom  sx={{textAlign:'center', fontSize: { xs: '0.75rem', sm: '1.5rem' } }}>نسخه پشتیبان</Typography>
      <Paper sx={{ p: 3, display: 'flex', gap: 2 }}>
        <Button variant="contained" onClick={handleSaveBackup}>ذخیره اطلاعات</Button>
