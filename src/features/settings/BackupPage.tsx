...
const BackupPage = () => {
  ...
  const handleSaveBackup = () => {
    try {
      ...
      link.download = `hesabyar-backup-${new Date()
        .toISOString()
        .slice(0, 10)}.json`;
  ...
  const handleConfirmRestore = () => {
    ...
    try {
      ...
      showToast(
        "اطلاعات با موفقیت بازیابی شد. لطفاً صفحه را رفرش کنید.",
        "success"
      );
    } catch (error: unknown) {
      ...
      <Box>
        <Paper sx={{ p: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            از تمام اطلاعات برنامه یک نسخه پشتیبان تهیه کنید یا اطلاعات خود را
            از یک فایل پشتیبان بازیابی نمایید.
          </Typography>
