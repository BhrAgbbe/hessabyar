...
const DataManagementPage = () => {
  ...
  const handleBackupAndPurge = async () => {
    ...
    try {
      ...
      const fileName = `hesabyar-backup-${new Date()
        .toISOString()
        .slice(0, 10)}.json`;
      ...
      showToast(
        `فرآیند پشتیبان‌گیری با خطا مواجه شد: ${errorMessage}`,
        "error"
      );
      ...
      <Box>
        <Paper sx={{ p: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            این عملیات یک نسخه پشتیبان از تمام اطلاعات برنامه دانلود کرده و سپس
            داده‌های فعلی را پاک می‌کند.
          </Typography>
