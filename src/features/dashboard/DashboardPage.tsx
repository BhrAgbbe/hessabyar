...
import { Grid, Typography, Paper, Button, Box, IconButton } from '@mui/material';
...
const ShortcutCard: React.FC<{ shortcut: Shortcut, onRemove: (id: string) => void }> = ({ shortcut, onRemove }) => {
  ...
  return (
    <Paper elevation={3} sx={{ p: 2, textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative' }}>
      ...
      <Box onClick={() => router.push(shortcut.path)} sx={{ cursor: 'pointer', flexGrow: 1 }}>
...
const SortableShortcutCard: React.FC<{ shortcut: Shortcut; onRemove: (id: string) => void }> = ({ shortcut, onRemove }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    <>
      ...
      <SortableContext items={shortcuts.map(s => s.id)} strategy={rectSortingStrategy}>
        <Grid
          ...
          container
          spacing={3}
          sx={{ ...dropZoneStyle, p: 2, minHeight: 400, alignContent: 'flex-start' }}
        >
